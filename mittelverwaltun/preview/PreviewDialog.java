...
public class PreviewDialog extends JDialog implements ActionListener {
    ...
    public PreviewDialog(String title, JFrame owner, Pageable pageable, double zoom) {
        ...
        toolbar.add(getButton("back.gif", new BrowseAction(preview, -1)));
        toolbar.add(getButton("forward.gif", new BrowseAction(preview, 1)));
        toolbar.add(new JToolBar.Separator());
        toolbar.add(getButton("zoomIn.gif", new ZoomAction(preview, DEFAULT_ZOOM_FACTOR_STEP)));
        toolbar.add(getButton("zoomOut.gif", new ZoomAction(preview, -DEFAULT_ZOOM_FACTOR_STEP)));
        toolbar.add(new JToolBar.Separator());
    ...
    private JButton getButton(String name, String iconName, AbstractAction action) {
        ...
        ImageIcon icon = null;
        URL imageURL = getClass().getClassLoader().getResource("image/" + iconName);
        if (imageURL != null)
